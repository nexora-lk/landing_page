          ...
          <div className="container" style={{ display: "flex", flexDirection: "column", gap: 24 }}>
            {services.map((svc) => (
              <Link key={svc.title} href={svc.href} className="service-row-link" style={{ padding: "clamp(32px, 4vw, 48px)", borderRadius: 20, background: "var(--surface)", border: "1px solid var(--border)", textDecoration: "none", color: "inherit", display: "flex", gap: 40, alignItems: "center", transition: "all 0.4s" }}>
                <div style={{ width: 56, height: 56, borderRadius: 14, background: "linear-gradient(135deg, rgba(0,212,255,0.15), rgba(124,92,255,0.15))", border: "1px solid var(--border)", display: "grid", placeItems: "center", fontSize: 24, flexShrink: 0 }}>{svc.icon}</div>
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                    {svc.tags.map((tag) => (
                      <span key={tag} style={{ padding: "4px 12px", borderRadius: 100, background: "rgba(0,212,255,0.08)", border: "1px solid rgba(0,212,255,0.2)", fontSize: 12, color: "var(--accent)", fontWeight: 500 }}>{tag}</span>
                    ))}
                  </div>
                </div>
